	...
	protected:
		...
		Ref<T> CloneNode(std::map<const GraphNode*, Ref<GraphNode>>& createdNodes, Args&&... args) const
		{
			...
			for (size_t i = 0; i < m_Inputs.size(); ++i)
			{
				...
				if (m_Variables[i])
				{
					if (m_Variables[i]->IsDefaultVar())
					{
						// This is the default variable that was generated by the graph (a user didn't connect the pin).
						// In this case, we need to perform a deep copy so that modifying this variable in the UI doesn't affect other existing graphs
						clone->m_Variables[i] = CopyVarByType(m_Variables[i]);
					}
					else
					{
