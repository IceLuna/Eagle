...
namespace Eagle
{
	...
	void BloomPassTask::InitPipeline()
	{
		// Downscale pipeline
		{
			...
			state.ComputeShader = Shader::Create("assets/shaders/bloom_downscale.comp", ShaderType::Compute);
			m_DownscalePipeline = PipelineCompute::Create(state);
		}
		// Upscale pipeline
		{
			PipelineComputeState state;
			state.ComputeShader = Shader::Create("assets/shaders/bloom_upscale.comp", ShaderType::Compute);
