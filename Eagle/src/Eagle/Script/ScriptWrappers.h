...
namespace Eagle::Script
{
	...
	float Eagle_CameraComponent_GetDirLightShadowFarClip(GUID entityID);
	void Eagle_CameraComponent_SetDirLightShadowFarClip(GUID entityID, float value);
	...
	void Eagle_Renderer_SetGTAOSettings(uint32_t samples, uint32_t stepsPerSample, float radius, float fallollRange, bool bGenerateBentNormals, bool bHalfRes);
	void Eagle_Renderer_GetGTAOSettings(uint32_t* outSamples, uint32_t* outStepsPerSample, float* outRadius, float* outFalloffRange, bool* outGenerateBentNormals, bool* outHalfRes);
	...
	bool Eagle_Renderer_IsGeometricSpecularAAEnabled();
	void Eagle_Renderer_SetGeometicSpecularAAEnabled(bool bEnabled);
	...
	void Eagle_Renderer_GetScreenSpaceReflectionsSettings(float* varianceThreshold, float* depthBufferThickness, float* temporalStabilityFactor, float* roughnessThreshold,
	...
	void Eagle_Renderer_SetScreenSpaceReflectionsSettings(float varianceThreshold, float depthBufferThickness, float temporalStabilityFactor, float roughnessThreshold,
		uint32_t samplesPerQuad, uint32_t maxTraversalIterations, uint32_t minTraversalOccupancy, bool bTemporalVarianceGuidedTracing, bool bEnabled);
	...
	void Eagle_AssetTexture2D_SetCompression(GUID id, TextureCompressor::Quality value);
	AssetTexture2DFormat Eagle_AssetTexture2D_GetFormat(GUID id);
	bool Eagle_AssetTexture2D_IsNormalMap(GUID id);
	TextureCompressor::Quality Eagle_AssetTexture2D_GetCompressionQuality(GUID id);
	...
	void Eagle_AssetMaterial_GetMaterial(GUID assetID,
		...
		glm::vec4* outTint, glm::vec3* outEmissiveIntensity, glm::vec2* outTilingFactor, MaterialBlendMode* outBlendMode, bool* bDoubleSided,
	...
	void Eagle_AssetMaterial_SetMaterial(GUID assetID,
		...
		bool bUseAlbedoTexture, bool bUseMetalnessTexture, bool bUseRoughnessTexture, bool bUseAOTexture, bool bUseEmissiveTexture, bool bUseOpacityTexture, bool bUseOpacityMaskTexture,
		const glm::vec4* tint, const glm::vec3* emissiveIntensity, const glm::vec2* tilingFactor, MaterialBlendMode blendMode, bool bDoubleSided,
