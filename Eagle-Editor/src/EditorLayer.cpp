	...
	void EditorLayer::DrawRendererSettings()
	{
		...
		// Shadow Resolutions settings
		{
			...
			if (treeOpened)
			{
				...
				bSettingsChanged |= UI::Property("Enable CSM smooth transition", options.bEnableCSMSmoothTransition, "Enable smooth transition of cascaded shadows (affects shadows that are casted by directional light)");
