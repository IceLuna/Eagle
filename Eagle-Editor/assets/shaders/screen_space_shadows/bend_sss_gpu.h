...
// Runtime data returned from BuildDispatchList():
    vec4 LightCoordinate; // Values stored in DispatchList::LightCoordinate_Shader by BuildDispatchList()
    ivec2 WaveOffset; // Values stored in DispatchData::WaveOffset_Shader by BuildDispatchList()
...
// Sampling data:
    vec2 InvDepthTextureSize; // Inverse of the texture dimensions for 'DepthTexture' (used to convert from pixel coordinates to UVs)
...
{
	ivec2 xy = inGroupID.yz * WAVE_SIZE + inParameters.WaveOffset.xy;
	...
	vec2 end_xy = mix(inParameters.LightCoordinate.xy, start_xy, vec2((major_axis_end + ma_light_frac) / (major_axis_start + ma_light_frac)));
	...
	vec2 pixel_xy = mix(start_xy, end_xy, vec2(thread_step / float(WAVE_SIZE)));
...
//	(float)	resultVisibility:	Output screen space shadow visibility
void WriteScreenSpaceShadow(DispatchParameters inParameters, ivec3 inGroupID, int inGroupThreadID, sampler2D depthTexture, out vec2 write_xy, out float resultVisibility)
{
	resultVisibility = 1.0;
	...
	for (i = 0; i < READ_COUNT; i++)
	{
		...
		depths.x = texture(depthTexture, read_xy * inParameters.InvDepthTextureSize).x;
		depths.y = texture(depthTexture, (read_xy + offset_xy) * inParameters.InvDepthTextureSize).x;
	#else
		int bias = bilinear > 0 ? 1 : -1;
		ivec2 offset_xy = ivec2(x_axis_major ? 0 : bias, x_axis_major ? bias : 0);
        // Return first component on the texture compared to original, due to texture format change
		depths.x = textureOffset(depthTexture, read_xy * inParameters.InvDepthTextureSize, ivec2(0)).x;
		depths.y = textureOffset(depthTexture, read_xy * inParameters.InvDepthTextureSize, offset_xy).x;
	#endif
	...
	if (inParameters.UseEarlyOut && (inParameters.DebugOutputWaveIndex == false && inParameters.DebugOutputThreadIndex == false && inParameters.DebugOutputEdgeMask == false))
	{
		...
		skip_pixel = EarlyOutPixel(inParameters, ivec2(write_xy), sampling_depth[0]);
	...
	shadow_value = clamp(shadow_value * (inParameters.ShadowContrast) + (1 - inParameters.ShadowContrast), vec4(0), vec4(1));
	hard_shadow = clamp(hard_shadow * (inParameters.ShadowContrast) + (1 - inParameters.ShadowContrast), 0, 1);
