#ifndef EG_POSTPROCESSING_ACES
#define EG_POSTPROCESSING_ACES
...
vec3 ACESTonemap(vec3 pixel)
{
    ...
    float d = +0.4329510f;
    float e = +0.238081f;
    color = ACES_fma(color, (color + a), b) / ACES_fma(color, ACES_fma(c, color, d), e);
